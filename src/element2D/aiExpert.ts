export class AIExpert {
  ...
  constructor() {
    this.htmlElement = document.getElementById("aiExpert") as HTMLDivElement;
  }
  ...
  public reset(): void {
    ...
    this.htmlElement.appendChild(document.createTextNode("AI expert says that..."));
  }
  ...
  private addBigLabel(label: string, content: string, htmlElement: any = document.createTextNode("")): void {
    this.addLabelInternal(label, document.createElement("h1"));
    this.addLabelContent(content, htmlElement);
  }

  private addLabel(label: string, content: string, htmlElement: any = document.createTextNode("")): void {
  ...
  public show(): void {
    ...
    fetch(`/server/askAI?latitude=${this.latitude}&longitude=${this.longitude}&craterRadiusMeters=${this.craterRadiusMeters}`).then((response: Response) => {
      response.text().then((aiResponse: string) => {
        fetch(`/server/aiQuickConclusion?wholeMessage=${aiResponse}`).then((shortResponse: Response) => {
          shortResponse.text().then((aiShortResponse: string) => {
            this.removeChildren();

            this.addBigLabel("AI expert's short conclusion: ", aiShortResponse);
  ...
  public setup(latitude: number, longitude: number, craterRadiusMeters: number) {
