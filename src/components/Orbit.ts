...
} from "../utility/orbitalCalculations";
import type { orbitElements } from "../utility/types";
...
class Orbit {
    ...
    public celestialObject: CelestialObject | Asteroid;
    ...
    constructor(
        ...
        celestialObject: CelestialObject | Asteroid,
        color: string,
    ...
    public visualize(origin: Vector3 | null = null): void {
        ...
        let i = 0;
        while (i <= 6.28 + 0.2) {
            points.push(this.calculatePosition(i, this.earthPos));
    ...
    public trace(_earthPos: Vector3): void {
        if (!this.orbitLine || !this.celestialObject) return;
        this.earthPos = _earthPos;
